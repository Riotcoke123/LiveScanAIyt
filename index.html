...
async function getChatAnalysis(chatId, durationSec) {
    ...
    // Heuristic list of AI-like response patterns
    ...
    while (Date.now() < endTime) {
        try {
            ...
            res.data.items?.forEach(item => {
                ...
                const messageText = item.snippet.displayMessage?.toLowerCase() || '';

                const hasBotPhrase = botLikePhrases.some(phrase => messageText.includes(phrase));

                if (uniqueChatAuthors.has(authorId)) {
                    const user = uniqueChatAuthors.get(authorId);
                    user.messageCount++;
                    if (hasBotPhrase) user.botPhraseHits++;
                    user.messages.push(messageText);
                } else {
                    uniqueChatAuthors.set(authorId, {
                        ...
                        botPhraseHits: hasBotPhrase ? 1 : 0,
    ...
    uniqueChatAuthors.forEach(user => {
        const msgSimilarity = new Set(user.messages).size / user.messages.length;

        const isSuspiciousSpammer = user.messageCount > SUSPICIOUSLY_HIGH_MESSAGE_COUNT_PER_USER && !user.isModerator && !user.isOwner;

        const isLikelyAiBot =
            user.botPhraseHits > 0 ||
            msgSimilarity < 0.5 ||
            (user.messageCount > 5 && user.messages.every(m => m.length > 15 && m.endsWith('.')));

        if (isSuspiciousSpammer) suspicious++;
        if (isLikelyAiBot && !user.isModerator && !user.isOwner) aiLikeBots++;
    });
...
async function runDetectionLoop() {
    ...
    const analyze = async () => {
        ...
        logToFile(entry);
        console.log(`[${timestamp}] Done. Real: ${est.estimatedRealViewers}, Bots: ${est.estimatedBotViewers}, AI-like Bots: ${chat.detectedAiLikeBots}`);
    };
...
// Hardcoded API key and channel ID
const API_KEY = 'AIzaSyAV1O8354AbkamXLGBXnmXlm0TwEfOdnjo';
const CHANNEL_ID = 'UCoxFxZirbfLvy9tres71eSA';
